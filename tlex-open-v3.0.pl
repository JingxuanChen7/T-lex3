...
sub generatefiles {
  ...
  print $script_file "awk -v chrnamelen=\"chrnamelen\" -F\$'\\t' '{filename = \"$wd\/chromosomes\/\" \$1; if (length(\$1) <=chrnamelen) print > filename }' $outputdir\/$strain\/$strainname\_bwa_MQ_pileups";
  close $script_file;
  # Execute it
